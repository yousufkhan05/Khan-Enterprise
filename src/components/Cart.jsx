  ...
  if (isOrdered) {
    return (
      <div className="max-w-md mx-auto my-12 p-8 bg-white dark:bg-slate-900 rounded-3xl shadow-xl border text-center border-slate-200 dark:border-slate-800">
        ...
        <button onClick={() => setPage('shop')} className="mt-6 w-full bg-blue-600 text-white font-bold py-3 rounded-xl text-xs uppercase cursor-pointer">Continue Shopping</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 text-slate-900 dark:text-slate-100 min-h-[65vh]">
      <div className="lg:col-span-7">
        <button onClick={() => setPage('shop')} className="flex items-center gap-1.5 text-xs font-black text-slate-400 mb-5 uppercase hover:text-blue-500 cursor-pointer"><ArrowLeft size={14} /> Return to Shop</button>
          ...
          <div className="space-y-4">
            {cart.map((item, idx) => (
              <div key={idx} className="flex items-center gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                {/* 📸 ইমেজ রেন্ডারিং ফিক্স কন্ডিশন */}
                <img src={item.images?.[0] || item.image} alt="" className="w-14 h-14 object-contain rounded-xl bg-white border p-1" />
